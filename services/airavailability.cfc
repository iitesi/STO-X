	...
	<cffunction name="doAvailability" output="false">
		...
		<cfif NOT structKeyExists(session.searches, arguments.Filter.getSearchID())
			...
			<!--- <cfthread
				...
				local.attributes.Group="#arguments.Group#";
				local.attributes.Account="#arguments.Account#";
				local.attributes.Policy="#arguments.Policy#";

 			  local.attributes.sNextRef = 'ROUNDONE';
				local.attributes.nCount = 0;
				</cfscript>
				<cfloop condition="local.attributes.sNextRef NEQ ''">
					<cfset local.attributes.nCount++>
					<!--- Put together the SOAP message. --->
					<cfset local.attributes.sMessage = prepareSoapHeader(arguments.Filter, arguments.Group, (local.attributes.sNextRef NEQ 'ROUNDONE' ? local.attributes.sNextRef : ''), arguments.Account, arguments.sCabins)>
					<!--- Call the getUAPI. --->
					<cfset local.attributes.sResponse = getUAPI().callUAPI('AirService', local.attributes.sMessage, arguments.Filter.getSearchID(), arguments.Filter.getAcctID(), arguments.Filter.getUserID())>
						<!--- <cfdump var="#local.attributes.sResponse#" abort/> --->
...
--->
<!---
<cfsavecontent variable="local.attributes.sResponse">
...
--->
					...
					<cfset local.attributes.aResponse = getUAPI().formatUAPIRsp(local.attributes.sResponse)>
						<!--- <cfdump var="#local.attributes.aResponse#"abort/> --->
					<!--- Create unique segment keys. --->
					<cfset local.attributes.sNextRef =	getAirParse().parseNextReference(local.attributes.aResponse)>
					<cfif local.attributes.nCount GT 3>
						<cfset local.attributes.sNextRef	= ''>
					</cfif>
					<!--- Create unique segment keys. --->
					<cfset local.attributes.stSegmentKeys = parseSegmentKeys(local.attributes.aResponse)>
					<!--- Add in the connection references --->
					<cfset local.attributes.stSegmentKeys = addSegmentRefs(local.attributes.aResponse, local.attributes.stSegmentKeys)>
					<!--- Parse the segments. --->
					<cfset local.attributes.stSegments = parseSegments(local.attributes.aResponse, local.attributes.stSegmentKeys)>
					<!--- Create a look up list opposite of the stSegmentKeys --->
					<cfset local.attributes.stSegmentKeyLookUp = parseKeyLookUp(local.attributes.stSegmentKeys)>
					<!--- Parse the trips. --->
					<cfset local.attributes.stAvailTrips = parseConnections(local.attributes.aResponse, local.attributes.stSegments, local.attributes.stSegmentKeys, local.attributes.stSegmentKeyLookUp, arguments.filter, arguments.group)>
					<!--- Add group node --->
					...
					<cfset local.attributes.stAvailTrips	= getAirParse().checkPolicy(local.attributes.stAvailTrips, arguments.Filter.getSearchID(), '', 'Avail', arguments.Account, arguments.Policy)>
					<!--- Create javascript structure per trip. --->
					<cfset local.attributes.stAvailTrips	=	getAirParse().addJavascript(local.attributes.stAvailTrips, 'Avail')>
					<!--- Merge information into the current session structures. --->
					<cfset session.searches[arguments.Filter.getSearchID()].stAvailTrips[arguments.Group] = getAirParse().mergeTrips(session.searches[arguments.Filter.getSearchID()].stAvailTrips[arguments.Group], local.attributes.stAvailTrips)>
				</cfloop>
