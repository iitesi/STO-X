...
<!--- session : search --->
	<cffunction name="search" access="remote" output="false" returntype="void">
		...
		<cfif NOT done OR refresh>
			...
			<cfset tab.stAvailDetails.stSortSegments[3] = []>
			...
			<cfif getsearch.Air AND getsearch.Air_Type EQ 'RT'>
				<cfif DateFormat(getsearch.Depart_DateTime) NEQ DateFormat(getsearch.Arrival_DateTime)>
					<cfset tab.sHeading = '<h3>'&getsearch.Depart_City&'-'&getsearch.Arrival_City&'</h3> '&DateFormat(getsearch.Depart_DateTime, 'ddd, m/d')&' to '&DateFormat(getsearch.Arrival_DateTime, 'm/d')>
				<cfelse>
					<cfset tab.sHeading = '<h3>'&getsearch.Depart_City&'-'&getsearch.Arrival_City&'</h3> '&DateFormat(getsearch.Depart_DateTime, 'ddd, m/d')>
				</cfif>
			...
			<cfelseif getsearch.Air AND getsearch.Air_Type EQ 'MD'>
				...
				<cfloop query="getsearchlegs">
					<cfset tab.sHeading = tab['Heading']&getsearchlegs.Depart_City&'-'&getsearchlegs.Arrival_City&' on '&DateFormat(getsearchlegs.Depart_DateTime, 'ddd, m/d')&' '>
					<cfset tab.stLegs[getsearchlegs.CurrentRow-1] = getsearchlegs.Depart_City&' to '&getsearchlegs.Arrival_City&' on '&DateFormat(getsearchlegs.Depart_DateTime, 'ddd, m/d')>
				</cfloop>
