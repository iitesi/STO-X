						...
						<ul>
							<li>
								...
								OR StructKeyExists(session.searches[rc.SearchID].stLowFareDetails.stPricing, 'YX')>
									<input type="checkbox" id="ClassY" name="ClassY" value="Y" <cfif NOT structKeyExists(rc, 'sCabins') OR rc.sCabins EQ 'Y' OR rc.filter EQ 'all'>checked</cfif>><label for="ClassY">Economy (#session.searches[rc.SearchID].stLowFareDetails.stResults.Y# results)</label>
								</cfif>
							</li>
							<li>
								<cfif structKeyExists(session.searches[rc.SearchID].stLowFareDetails.stResults, "C")>
									<input type="checkbox" id="ClassC" name="ClassC" value="C" <cfif (structKeyExists(rc, 'sCabins') AND rc.sCabins EQ 'C') OR rc.filter EQ 'all'>checked</cfif>><label for="ClassC">Business (#session.searches[rc.SearchID].stLowFareDetails.stResults.C# results)</label>
							...
							<li>
								<cfif structKeyExists(session.searches[rc.SearchID].stLowFareDetails.stResults, "F")>
									<input type="checkbox" id="ClassF" name="ClassF" value="F" <cfif (structKeyExists(rc, 'sCabins') AND rc.sCabins EQ 'F') OR rc.filter EQ 'all'>checked</cfif>><label for="ClassF">First (#session.searches[rc.SearchID].stLowFareDetails.stResults.F# results)</label>
						...
						<ul>
							<li>
								...
								OR StructKeyExists(session.searches[rc.SearchID].stLowFareDetails.stPricing, 'X0')>
									<input type="checkbox" id="Fare0" name="Fare0" value="0" <cfif (NOT structKeyExists(rc, 'bRef') OR rc.bRef EQ 0) OR rc.filter EQ 'all'>checked</cfif>><label for="Fare0">Non Refundable (#session.searches[rc.SearchID].stLowFareDetails.stResults[0]# results)</label>
								</cfif>
							</li>
							<li>
								...
								OR StructKeyExists(session.searches[rc.SearchID].stLowFareDetails.stPricing, 'X1')>
									<input type="checkbox" id="Fare1" name="Fare1" value="1" <cfif (structKeyExists(rc, 'bRef') AND rc.bRef EQ 0) OR rc.filter EQ 'all'>checked</cfif>><label for="Fare1">Refundable (#session.searches[rc.SearchID].stLowFareDetails.stResults[1]# results)</label>
								</cfif>
