	...
	<cfif structKeyExists(session.searches[rc.Search_ID].stAvailDetails.aSortDuration, rc.Group)>
		...
		<cfoutput>
			<cfsavecontent variable="jsondata">
				<cfset Count = 0>
				<cfloop array="#session.searches[rc.nSearchID].stAvailDetails.aSortDuration[rc.Group]#" index="i">
					<cfset TotalSegments = ArrayLen(StructKeyArray(session.searches[rc.nSearchID].stAvailTrips[rc.nGroup][i].Groups[0].Segments))>
					<cfset SegmentCount = 0>
					<cfloop collection="#session.searches[rc.nSearchID].stAvailTrips[rc.nGroup][i].Groups[0].Segments#" index="j">
						<cfset Count++>
						<cfset SegmentCount++>
						<cfset stSegment = session.searches[rc.nSearchID].stAvailTrips[rc.nGroup][i].Groups[0].Segments[j]>
						...
						<cfif TotalSegments NEQ SegmentCount>
							,{
								...
								<cfset stNextSegment = session.searches[rc.nSearchID].stAvailTrips[rc.nGroup][i].Groups[0].Segments[j+1]>
