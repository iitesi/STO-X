...
<!--- to do - loop through the traveler structure --->
		<cfset local.travelerNumber = 1>
		...
		<cfset local.Traveler = session.searches[rc.searchID].Travelers[travelerNumber]>
		...
		<cfif hotelSelected
			...
			<!--- Parse error --->
			<cfif Hotel.UniversalLocatorCode EQ ''>
		...
		<cfif vehicleSelected
			...
			<!--- Parse error --->
			<cfif Vehicle.UniversalLocatorCode EQ ''>
