<cfcomponent extends="abstract" accessors="true">
	...
	<cffunction name="lowfareavail" output="false" hint="I assemble low fares for display.">
		...
		<cfif IsStruct(rc.Policy) 
			AND rc.Policy.Policy_AirRefRule EQ 1 
			AND rc.Policy.Policy_AirNonRefRule EQ 0>
		...
		<cfif structKeyExists(arguments.rc, "filter")AND dateDiff('d', now(), arguments.rc.filter.getDepartDateTime()) GTE 0>
			...
			<cfif NOT structKeyExists(arguments.rc, 'bSelect')> <!---IF WE HAVEN'T SELECTED A FLIGHT--->
				...
				<cfset local.stTrips = variables.lowfare.threadLowFare(argumentcollection=arguments.rc)>
				
				<!--- Air availability --->
				<cfset local.stAvailTrips[arguments.rc.Group] = variables.airavailability.doAvailabilityNew(Filter = arguments.rc.Filter,
																											Group = arguments.rc.Group,
																											Account = arguments.rc.Account,
				...
				<!--- Create the linking table between stTrips and stAvailTrips --->
				<cfset rc.stLowFareAvail =  variables.lowfareavail.createLowFareAvail(	stTrips = local.stTrips,
																						stAvailTrips = local.stAvailTrips[arguments.rc.Group],
																						Group = rc.Group )>
			</cfif>
		...
		<cfset rc.totalFlights = structCount(local.stAvailTrips[rc.Group])>
		<cfset rc.stTrips = local.stTrips>
		<cfset rc.stAvailTrips = local.stAvailTrips[rc.Group]>
